<?php
// tratamientotermico_reporte.php - muestra grilla de todos los registros de tratamiento térmico
...
$sql = "SELECT 
            ...
            t.Estacion, 
            t.Lote, 
            t.CorrelativoNumerico,
            t.EspecieMaderaTratada, 
            t.Codigo,
            t.Producto, 
            -- Determinar Familia (Parihuelas o Listones)
            CASE 
                WHEN LEFT(t.Lote, 1) = 'P' THEN 'PARIHUELAS'
                ELSE 'LISTONES'
            END AS Familia,
            t.CantidadRumas, 
            t.CantidadUnidades, 
            -- Metros Cúbicos: suma de m³ de pallets relacionados
            COALESCE(
                (SELECT SUM(p.MetrosCubicos)
                 FROM scm_registrotratamiento_pallets p
                 WHERE p.IdTratamiento = t.Id
                ),
                0
            ) AS MetrosCubicos,
            -- Lote Etiquetado (interno, no se muestra pero se usa para el cruce)
            COALESCE(
                (SELECT ch.numero_tratamiento 
                 FROM control_horno ch
                 WHERE CAST(SUBSTRING_INDEX(t.Lote, '-', -1) AS UNSIGNED) = CAST(SUBSTRING_INDEX(ch.numero_tratamiento, '-', 1) AS UNSIGNED)
                 AND (
                     (LEFT(t.Lote, 1) = 'P' AND ch.producto LIKE '%PARIHUELA%')
                     OR
                     (LEFT(t.Lote, 1) != 'P' AND ch.producto NOT LIKE '%PARIHUELA%')
                 )
                 LIMIT 1
                ), 
                ''
            ) AS LoteEtiquetado,
            -- Q Etiquetado: suma de cantidades de lotes que coinciden
            COALESCE(
                ...
                0
            ) AS CantidadEtiquetado,
            t.Observaciones,
            t.estado,
            t.registro_timestamp,
            t.registro_usuario
        FROM scm_registrotratamientotermico t
        WHERE t.FechaTratamiento >= '2026-02-01'
        ORDER BY t.FechaTratamiento DESC, t.Id DESC";
$res = $conn->query($sql);
$rows = [];
...
// VALIDACIÓN REMOTA DESHABILITADA
// La comprobación contra el hosting quedó comentada a pedido del usuario.
// Si se desea reactivar en el futuro, restaurar la lógica que consulta
// `sync_horno_export.php` por codigo_pallet/lote y compara cantidades.
$validaciones = []; // mantenemos la variable por compatibilidad
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
<title>Reporte Tratamiento Térmico</title>
<style>
    ...
    #grid thead th.col-lote { width: 120px; }
    #grid thead th.col-fecha { width: 160px; }
    #grid tbody tr td:nth-child(2) { white-space: nowrap; width: 160px; }
    /* Opcionales: permitir que la columna Observaciones sea más ancha si hay espacio */
    #grid thead th.col-observaciones { width: 28%; }
    ...
    /* Estado: colores según estado */
    .estado { font-weight: 500; text-transform: uppercase; }
    .estado-activo { color: #388e3c; } /* verde */
    .estado-inactivo { color: #757575; } /* gris */
    /* Modal confirmación */
...
<body>
    <h2>Reporte de Tratamiento Térmico HT-144</h2>
    <div class="controls">
        <div class="controls-left">
            ...
            <button id="btn-panel" onclick="location.href='panel.html'">Panel</button>
    ...
    <table id="grid">
        ...
        <tr>
            ...
            <th class="col-fecha">Fecha T.T.</th>
            <!-- <th class="col-estacion">Estación</th> -->
            <th class="col-lote">Lote T.T.</th>
            <!-- <th class="col-especie">Especie Madera</th> -->
            <!-- <th class="col-codigo">Código</th> -->
            <!-- <th class="col-producto">Producto</th> -->
            <th class="col-familia">Familia</th>
            <!-- <th class="col-rumas">Rumas</th> -->
            <th class="col-unidades">Q Unidades</th>
            <th class="col-m3">Q m³</th>
            <!-- <th class="col-loteetiquetado">Lote (Etiquetado)</th> -->
        ...
        <tbody>
            ...
            <tr>
                <?php
                    ...
                    $estadoRaw = isset($r['estado']) ? strtoupper(trim($r['estado'])) : 'ACTIVO';
                    $estadoClass = 'estado';
                    if ($estadoRaw === 'ACTIVO') $estadoClass .= ' estado-activo';
                    else $estadoClass .= ' estado-inactivo';
                ?>
                <td><?php echo htmlspecialchars($r['Id'] ?? ''); ?></td>
                <td><?php echo htmlspecialchars((!empty($r['FechaTratamiento']) && strtotime($r['FechaTratamiento'])) ? date('d-m-Y', strtotime($r['FechaTratamiento'])) : $r['FechaTratamiento']); ?></td>
                <!-- <td><?php echo htmlspecialchars($r['Estacion'] ?? ''); ?></td> -->
                <td><?php echo htmlspecialchars($r['Lote'] ?? ''); ?></td>
                <!-- <td><?php echo htmlspecialchars($r['EspecieMaderaTratada'] ?? ''); ?></td> -->
                ...
                <!-- <td><?php echo htmlspecialchars($r['CantidadRumas'] ?? ''); ?></td> -->
                <td><?php echo htmlspecialchars($r['CantidadUnidades'] ?? ''); ?></td>
                <?php
                    // Mostrar m³ desde la base de datos (suma de MetrosCubicos de pallets)
                    $m3_value = isset($r['MetrosCubicos']) ? floatval($r['MetrosCubicos']) : 0;
                    $m3_display = ($m3_value > 0) ? number_format($m3_value, 2, '.', '') : '';
                ?>
                <td><?php echo htmlspecialchars($m3_display); ?></td>
                <!-- <td><?php echo htmlspecialchars($r['LoteEtiquetado'] ?? ''); ?></td> -->
                <td>CONFORME</td>
                <td><span class="<?php echo $estadoClass; ?>"><?php echo htmlspecialchars($estadoRaw); ?></span></td>
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    var STORAGE_KEY = 'tt_column_filters_v1';
