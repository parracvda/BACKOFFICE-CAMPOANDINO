<?php
// reporte.php - muestra grilla de todos los requerimientos desde estructura V2
...
<style>
    ...
    #grid thead th.col-rq { width: 90px; }
    /* Aumentar ancho de Fecha y forzar no-wrap en la columna (3) */
    #grid thead th.col-fecha { width: 160px; }
    #grid tbody tr td:nth-child(3) { white-space: nowrap; width: 160px; }
    ...
    /* Prioridad: mostrar en mayúsculas y color según valor */
    .prio { font-weight: 600; text-transform: uppercase; }
    .prio-normal { color: #2e7d32; } /* verde */
    .prio-alta { color: #d32f2f; }   /* rojo */
    /* Estado: colores según estado de aprobación */
    .estado { font-weight: 500; }
    .estado-pendiente { color: #f57c00; } /* naranja */
    .estado-aprobado { color: #1976d2; }  /* azul */
    .estado-entregado { color: #388e3c; } /* verde oscuro */
    /* Modal confirmación */
...
<body>
    ...
    <table id="grid">
        ...
        <tr>
            ...
            <th class="col-npallets">NPallets</th>
        ...
        <tbody>
            ...
            <tr>
                <?php
                    $prioRaw = isset($r['Prioridad']) ? $r['Prioridad'] : '';
                    $prioVal = strtoupper(trim((string)$prioRaw));
                    $prioClass = 'prio';
                    if (strtolower(trim((string)$prioRaw)) === 'alta') $prioClass .= ' prio-alta';
                    else if (strtolower(trim((string)$prioRaw)) === 'normal') $prioClass .= ' prio-normal';
                    
                    // Determinar clase para estado
                    $estadoRaw = isset($r['EstadoAprobacion']) ? $r['EstadoAprobacion'] : 'Pendiente';
                    $estadoClass = 'estado';
                    if ($estadoRaw === 'Aprobado') $estadoClass .= ' estado-aprobado';
                    else if ($estadoRaw === 'Entregado') $estadoClass .= ' estado-entregado';
                    else $estadoClass .= ' estado-pendiente';
                ?>
                <td><span class="<?php echo $prioClass; ?>"><?php echo htmlspecialchars($prioVal); ?></span></td>
                ...
                <td><?php echo htmlspecialchars($r['NPallets'] ?? ''); ?></td>
    ...
    <script>
// Filtrado cliente con filtros por columna (checkboxes) + búsqueda global
document.addEventListener('DOMContentLoaded', function(){
    ...
    var STORAGE_KEY = 'rq_column_filters_v1';
    ...
    // Si respectOtherFilters=true se toman en cuenta los filtros ya aplicados
    // (excepto la columna objetivo) para listar solo valores disponibles
    ...
    // Botones
        ...
        // asegurar que el botón y badge estén disponibles para actualizar estado
        if (!btn) {
            // intentar encontrar el botón agregado en th
            btn = th.querySelector('.col-filter-btn');
            badge = btn ? btn.querySelector('.col-filter-badge') : null;
        }

        th.appendChild(dropdown);

        // helper para actualizar estado visual del botón
        function updateButtonState(count){
            if (!btn) return;
            if (count > 0) {
                btn.classList.add('active');
                if (badge) { badge.style.display = 'inline-block'; badge.innerText = String(count); }
            } else {
        ...
        // Inicializar estado según selectedFilters existente
        var initialCount = 0;
        if (selectedFilters[colIndex]) initialCount = selectedFilters[colIndex].size;
        updateButtonState(initialCount);
    }
