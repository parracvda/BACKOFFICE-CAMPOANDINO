...
if (trim($usuarioNombre) === '') {
  header('Location: acceso.php?error=' . urlencode('Debe iniciar sesión para acceder a esta página.'));
  exit;
}
...
<body>
  ...
  <script>
    ...
    function checkPalletExists(codigo) {
      ...
      return fetch('api_check_pallet.php?codigo=' + encodeURIComponent(codigo), { cache: 'no-store' })
    ...
    function _procesarCodigoQRBody(index, datosQR){
      ...
      fetch('api_buscar_almacenregistros.php?q=' + encodeURIComponent(datosQR.codigo))
        .then(function(res){ 
    ...
    function inicializarAutocomplete(index){
      ...
      input.addEventListener('input', function(){
        ...
        timeout = setTimeout(function(){
          ...
          fetch('api_buscar_almacenregistros.php?q=' + encodeURIComponent(q))
            .then(function(res){ 
    ...
    function actualizarPreviewLote(especie){
      ...
      fetch('api_preview_lote.php?especie=' + encodeURIComponent(especie))
        .then(function(res){ return res.json(); })
        .then(function(data){
    ...
    (function(){
      ...
      if (!p) return;
      p.addEventListener('click', function(){ window.location.href = 'panel.html'; });
    })();
