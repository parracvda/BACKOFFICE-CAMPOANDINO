...
if ($q !== '') {
    ...
    if (count($tokens) > 0) {
        ...
        $sql = "SELECT Codigo AS Id, Codigo, Producto AS Descripcion, SUM(Cantidad) AS Cantidad, COUNT(*) AS Pallets, GROUP_CONCAT(DISTINCT Observacion SEPARATOR ' | ') AS Observacion FROM stock_materiales WHERE " . $where . " GROUP BY Codigo, Producto LIMIT ?";
