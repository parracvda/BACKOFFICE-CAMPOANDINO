...
function cmbTipoProducto($conn) {
    $sql = "SELECT * FROM sig_vcc_tipoproducto";
    $result = $conn->query($sql);
    if (!$result) {
        die("Error en la consulta: " . $conn->error);
    }
...
function cmbDestino($conn) {
    $sql = "SELECT * FROM sig_vcc_destino";
    $result = $conn->query($sql);
    if (!$result) {
        die("Error en la consulta: " . $conn->error);
    }
...
function cmbCliente($conn) {
    $sql = "SELECT * FROM sig_vcc_cliente";
    $result = $conn->query($sql);
    if (!$result) {
        die("Error en la consulta: " . $conn->error);
    }
...
function cmbMateriaPrima($conn) {
    $sql = "SELECT * FROM sig_vcc_materiaprima";
    $result = $conn->query($sql);
    if (!$result) {
        die("Error en la consulta: " . $conn->error);
    }
...
function cmbObservacionPaletizado($conn) {
    $sql = "SELECT * FROM sig_vcc_obspal_pt";
    $result = $conn->query($sql);
    if (!$result) {
        die("Error en la consulta: " . $conn->error);
    }
...
function cmbTipoMaterial($conn) {
    $sql = "SELECT * FROM sig_vcc_tipomaterial_pt";
    $result = $conn->query($sql);
    if (!$result) {
        die("Error en la consulta: " . $conn->error);
    }
...
function cmbObservacionMuestra($conn) {
    $sql = "SELECT * FROM sig_vcc_obsmues_pt";
    $result = $conn->query($sql);
    if (!$result) {
        die("Error en la consulta: " . $conn->error);
    }
...
function sig_registrocontrolcalidad_agregar_buck($conn,$as_parametros) {
    ...
    $sql =
    "
    INSERT INTO `sig_registrocontrolcalidad_pt` 
    (`IdRegistroControlCalidad`, `Sede`, `Fecha`, `Turno`, `HoraInicio`, `HoraFin`, `Campana`, `TipoProducto`
    , `Destino`, `Cliente`, `MateriaPrima`, `CodigoPallet`, `LoteProduccion`, `LineaEnsamblaje`
...
function sig_registrocontrolcalidad_agregar($conn,$as_parametros) {  
    ...
    $stmt = $conn->prepare("CALL usp_sig_registrocontrolcalidad_pt(?)");
...
function obtenerPorID($conn,$as_parametros) {
    $sql = "SELECT * FROM sig_registrocontrolcalidad_pt where IdRegistroControlCalidad = " . $as_parametros;
    $result = $conn->query($sql);
    if (!$result) {
        die("Error en la consulta: " . $conn->error);
    }
...
try {
    ...
    if ($action == 'usuarios') {
        ...
        $datos = cmbTipoProducto($conn);
    } elseif ($action == 'cmbDestino') {
        $datos = cmbDestino($conn); 
    } elseif ($action == 'cmbCliente') {
        $datos = cmbCliente($conn); 
    } elseif ($action == 'cmbMateriaPrima') {
        $datos = cmbMateriaPrima($conn); 
    } elseif ($action == 'cmbCodigoPallet') {
        $datos = cmbCodigoPallet($conn); 
    } elseif ($action == 'cmbLoteProduccion') {
        $datos = cmbLoteProduccion($conn);
    } elseif ($action == 'cmbLineaEnsamblaje') {
        $datos = cmbLineaEnsamblaje($conn);
    } elseif ($action == 'cmbCantidad') {
        $datos = cmbCantidad($conn);
    } elseif ($action == 'cmbCodigoProducto') {
        $datos = cmbCodigoProducto($conn);
    } elseif ($action == 'cmbDescripcionProducto') {
        $datos = cmbDescripcionProducto($conn);
