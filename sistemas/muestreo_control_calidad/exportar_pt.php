...
try {
    ...
    $query = "SELECT rc.IdRegistroControlCalidad, se.Sedes, rc.Fecha, tu.Turno, rc.HoraInicio, rc.HoraFin, ca.Campana, tp.TipoProducto
        , de.Destino, cl.Cliente, mp.MateriaPrima, rc.CodigoPallet, rc.LoteProduccion, rc.LineaEnsamblaje
        ...
        FROM sig_registrocontrolcalidad_pt rc
        ...
        LEFT JOIN sig_vcc_cliente cl
        ON rc.Cliente = cl.IdCliente
        LEFT JOIN sig_vcc_materiaprima mp
        ON rc.MateriaPrima = mp.IdMateriaPrima
        LEFT JOIN sig_vcc_obspal_pt op
        ...
        $query = "SELECT rc.idRegistroControlCalidad, se.Sedes, rc.Fecha, rc.HoraInicio, tp.TipoProducto
        FROM sig_registrocontrolcalidad_pt rc
        LEFT JOIN general_sedes se
        ON rc.Sede = se.IdSedes
        LEFT JOIN sig_vcc_tipoproducto tp
    ...
    if (!empty($_GET['tipoMaterial_pt'])) {
        $conditions[] = "tm.TipoMaterial_pt = :tipoMaterial_pt";
        $params[':tipoMaterial_pt'] = $_GET['tipoMaterial_pt'];
    }
