...
$allowed = false;
// Permisos: solo jparra o quien tenga maintenance_access o quien haya cargado la UI de mantenimiento
...
// También permitir si la página de mantenimiento marcó la sesión como autorizada
if (!$allowed) {
    try { if (isset($_SESSION['can_maintain']) && $_SESSION['can_maintain'] === true) $allowed = true; } catch(Exception $e) {}
}

if (!$allowed) {
    error_log('api_set_user_feature: permisos insuficientes para usuario=' . ($currentUser ?? 'NULL') . ' id=' . intval($currentId));
